...
def start_measure():
    ...
    for i in range(torch.mlu.device_count()):
        measures[str(i)] = torch.mlu.memory_allocated(i)
...
def end_measure(start_measures):
    ...
    # GPU mem
    for i in range(torch.mlu.device_count()):
        measures[str(i)] = (torch.mlu.memory_allocated(i) - start_measures[str(i)]) / 2**20
        measures[f"{i}-peak"] = (torch.mlu.max_memory_allocated(i) - start_measures[str(i)]) / 2**20
