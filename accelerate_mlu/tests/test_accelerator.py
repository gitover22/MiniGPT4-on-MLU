...
from accelerate.test_utils.testing import AccelerateTestCase, require_mlu
...
class AcceleratorTester(AccelerateTestCase):
    ...
    def test_accelerator_can_be_reinstantiated(self):
        ...
        assert PartialState._shared_state["device"].type == "mlu"
    ...
    def test_env_var_device(self):
        ...
        # Mock torch.mlu.set_device to avoid an exception as the device doesn't exist
        def noop(*args, **kwargs):
            pass

        with patch("torch.mlu.set_device", noop), patch_environment(ACCELERATE_TORCH_DEVICE="mlu:64"):
            accelerator = Accelerator()
            self.assertEqual(str(accelerator.state.device), "mlu:64")
