...
class MultiGPUTester(unittest.TestCase):
    ...
    def test_multi_gpu(self):
        print(f"Found {torch.mlu.device_count()} devices.")
        cmd = get_launch_prefix() + [f"--nproc_per_node={torch.mlu.device_count()}", self.test_file_path]
    ...
    def test_multi_gpu_ops(self):
        print(f"Found {torch.mlu.device_count()} devices.")
        cmd = get_launch_prefix() + [f"--nproc_per_node={torch.mlu.device_count()}", self.operation_file_path]
        print(f"Command: {cmd}")
        with patch_environment(omp_num_threads=1):
            execute_subprocess_async(cmd, env=os.environ.copy())
    ...
    def test_pad_across_processes(self):
        cmd = get_launch_prefix() + [f"--nproc_per_node={torch.mlu.device_count()}", inspect.getfile(self.__class__)]
        with patch_environment(omp_num_threads=1):
            execute_subprocess_async(cmd, env=os.environ.copy())
    ...
    def test_distributed_data_loop(self):
        ...
        """
        print(f"Found {torch.mlu.device_count()} devices, using 2 devices only")
        cmd = get_launch_prefix() + [f"--nproc_per_node={torch.mlu.device_count()}", self.data_loop_file_path]
        with patch_environment(omp_num_threads=1, mlu_visible_devices="0,1"):
            execute_subprocess_async(cmd, env=os.environ.copy())
