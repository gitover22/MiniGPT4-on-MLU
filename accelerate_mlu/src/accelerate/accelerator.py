...
class Accelerator:
    ...
    Args:
        ...
        rng_types (list of `str` or [`~utils.RNGType`]):
            ...
            - `"mlu"`: the CUDA random number generator (GPU only)
    ...
    **Available attributes:**
    ...
    def prepare_model(self, model: torch.nn.Module, device_placement: bool = None, evaluation_mode: bool = False):
            ...
            if torch.device(current_device_index) != self.device:
                ...
                if (self.device.index is not None) or (current_device_index != 0):
                    raise ValueError(
                        ...
                        "you're training on. Make sure you loaded the model on the correct device using for example `device_map={'':torch.mlu.current_device()}"
                        "you're training on. Make sure you loaded the model on the correct device using for example `device_map={'':torch.mlu.current_device() or device_map={'':torch.xpu.current_device()}"
                    )
        ...
        if self.native_amp:
            ...
            mlu_device_capacity = torch.mlu.get_device_capability()
            fp8_enabled = mlu_device_capacity[0] >= 9 or (
                mlu_device_capacity[0] == 8 and mlu_device_capacity[1] >= 9
            )
            if not fp8_enabled:
                logger.warn(
                    f"The current device has compute capability of {mlu_device_capacity} which is "
