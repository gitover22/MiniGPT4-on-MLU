...
def require_cpu(test_case):
    ...
    """
    return unittest.skipUnless(not torch.mlu.is_available(), "test requires only a CPU")(test_case)
...
def require_mlu(test_case):
    ...
    """
    return unittest.skipUnless(torch.mlu.is_available(), "test requires a GPU")(test_case)
...
def require_single_gpu(test_case):
    ...
    """
    return unittest.skipUnless(torch.mlu.device_count() == 1, "test requires a GPU")(test_case)
...
def require_multi_gpu(test_case):
    ...
    GPUs.
    """
    return unittest.skipUnless(torch.mlu.device_count() > 1, "test requires multiple GPUs")(test_case)
